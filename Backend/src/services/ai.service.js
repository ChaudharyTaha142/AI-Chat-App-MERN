...
// Check if the Google API key is loaded from the .env file
const apiKey = process.env.GOOGLE_API_KEY;
if (!apiKey) {
    throw new Error("FATAL ERROR: GOOGLE_API_KEY is not defined in your .env file.");
}
...
async function generateResponse(content) {
    ...
    if (!content || content.length === 0) {
        return "I received an empty message. Could you please try sending that again?";
    }

    const chat = model.startChat({ history: content.slice(0, -1) });
    const lastMessage = content[content.length - 1].parts[0].text;
...
/**
 * Generates a vector embedding for a given text content.
 * @param {string} textContent - The text to be embedded.
 * @returns {Promise<number[]>} - A promise that resolves to an array of numbers representing the vector.
 */
async function generateVector(textContent) {
    const model = ai.getGenerativeModel({ model: "text-embedding-004" });

    // --- THIS IS THE FIX ---
    // The Google API requires the text to be wrapped in a 'Content' object.
    // We create that structure here before sending the request.
