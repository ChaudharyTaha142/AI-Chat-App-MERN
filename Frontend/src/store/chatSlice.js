...
const chatSlice = createSlice({
    ...
    reducers: {
        ...
        removeChat(state, action) {
            ...
            state.chats = state.chats.filter(c => c._id !== id && c.id !== id);
            if (state.activeChatId === id) state.activeChatId = state.chats.length ? (state.chats[0]._id || state.chats[0].id) : null;
        },
        addUserMessage: {
            reducer(state, action) {
